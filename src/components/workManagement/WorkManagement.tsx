...
export function WorkManagement({ isOpen, onClose }: WorkManagementProps) {
    ...
    try {
      ...
      await updateCompanyData({
        workManagement: {
          ...
          tasks: [...currentTasks, ...tasksWithCompanyId]
        }
      });
  ...
  const handleUpdateTaskStatus = async (taskId: string, status: 'todo' | 'completed', initials?: string) => {
    if (!selectedCompany?.id) return;
    ...
    try {
      ...
      const updatedTasks = tasks.map(task => 
        task.id === taskId ? {
          ...
          completedBy: status === 'completed' ? initials : undefined
        } : task
      );
